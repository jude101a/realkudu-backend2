...
// import {FIELD_MAP, buildFilters} from "./gen.property.model.js";
...
//     const { rows } = await pool.query(
//       `SELECT * FROM ${TABLE} WHERE property_id = $1 AND is_estate = false LIMIT 1`,
//       [propertyId]
//     );
//     return rows[0] || null;
//   }
...
//     const whereClause = built.conditions.length ? `WHERE deleted_at = null AND ${built.conditions.join(" AND ")}` : "";
//     const { rows } = await pool.query(
