...
async function createCoreTables(client) {
  ...
  await client.query(`
    CREATE TABLE IF NOT EXISTS sellers (
      ...
      seller_type VARCHAR(50) NOT NULL DEFAULT 'INDIVIDUAL',
      ...
      cac_document_url TEXT,
       memorandum_document_url TEXT,
      utilityBillDocumentUrl TEXT,
      businessProfileImageUrl TEXT,
      additionalShareholders TEXT,
      businessLga VARCHAR(100),
      businessState VARCHAR(100),
      businessWebsite VARCHAR(255),
      businessDescription TEXT,
      business_specification TEXT,
      business_profile_image_url TEXT,
      primary_shareholder_name VARCHAR(150),
      primary_shareholder_email VARCHAR(255) UNIQUE,
      primary_shareholder_phone VARCHAR(20),
      primary_shareholder_id_document_url TEXT,
      primary_shareholder_nin VARCHAR(11) UNIQUE,
      is_verified BOOLEAN DEFAULT FALSE,
...
async function createPropertyTables(client) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS estates (
      ...
      state VARCHAR(100),
      cover_image_url TEXT,
      estate_type VARCHAR(150) CHECK (estate_type IN ('land', 'building', 'combination')),
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      deleted_at TIMESTAMPTZ
    );
...
async function ensurePropertyTableHotfix(client) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS property (
      ...
      price_per_plot NUMERIC(12,2) DEFAULT 0 CHECK (price_per_plot>=0),
      price_per_450sqm NUMERIC(12,2) DEFAULT 0 CHECK (price_per_450sqm >= 0),
      price_per_900sqm NUMERIC(12,2) DEFAULT 0 CHECK (price_per_900sqm >= 0),
      price_per_customsqm NUMERIC(12,2) DEFAULT 0 CHECK (price_per_customsqm >= 0),
      custom_size NUMERIC,
